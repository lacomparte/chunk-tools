...
export const generateConfigCode = (
  ...
  addConfigHeader(lines, cacheKey);
  addChunkGroupsArray(lines, suggestions);
  lines.push('');
 ...
 * 설정 파일 헤더 추가
 */
const addConfigHeader = (lines: string[], cacheKey?: string): void => {
 ...
 */
// eslint-disable-next-line max-lines-per-function -- Complex table formatting for budget violations
